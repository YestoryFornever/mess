...
a.x和a先被提出来，但是.运算符优先级高于赋值运算符，所以先创建a.x=undefined 然后连等赋值从右到左 先执行a={n:2}，此时a的引用已经改变，但是内存里保存着的a还是之前和b引用相同地址的a，所以就变成了对b增添属性，即{n:1,x:{n:2}}
...
18	*将字符串"<tr><td>{$id}</td><td>{$name}</td></tr>"中的{$id}替换成10，{$name}替换成Tony （使用正则表达式）
		...
		tmpstr.replace(/{\$id}/g, '10').replace(/{\$name}/g, 'Tony');
...
47.	已知有字符串foo="get-element-by-id",写一个function将其转化成驼峰表示法"getElementById"。
	...
	#str.replace( /(p1)(p2)(p3)(p4)/ig, function(p1234, p1, p2, p3, ..., location-index, orign-str){});

48.	下面这个ul，如何点击每一列的时候alert其index?（闭包）
	XHTML
	<ul id="test">
		<li>这是第一条</li>
		<li>这是第二条</li>
